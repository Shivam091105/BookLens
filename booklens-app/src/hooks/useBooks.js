...
export function useLogBook() {
  ...
  return useMutation({
    mutationFn: ({ bookExternalId, payload }) => logsApi.logBook(bookExternalId, payload),
    onSuccess: (data, { bookExternalId, payload }) => {
      // Mark book detail as stale WITHOUT triggering an immediate refetch.
      // refetchType:'none' means: "stale it so next mount fetches fresh data,
      // but do not re-render right now." This prevents the sync re-render that
      // was crashing the page when rating distribution data was present.
      queryClient.invalidateQueries({
        queryKey: keys.books.detail(bookExternalId),
        refetchType: 'none',
      })
      queryClient.invalidateQueries({ queryKey: keys.logs.diary() })
      queryClient.invalidateQueries({ queryKey: keys.logs.feed })
