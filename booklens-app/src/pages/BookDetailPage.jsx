...
// import { useBookReviews, useCreateReview, useUpdateReview, useDeleteReview } from '../hooks/useReviews'
...
//   // Determine author display — filter out "See Open Library" placeholder from cached responses
//   const rawAuthors = book.authors && book.authors.length > 0
//     ? book.authors.filter(a => a && a !== 'See Open Library')
//     : []
//   const allAuthors = rawAuthors.length > 0
//     ? rawAuthors
//     : book.author && book.author !== 'See Open Library' ? [book.author] : ['Unknown Author']
...
//                 {reviews.map(r => {
//                   const rUsername = r.username ?? r.user?.username ?? 'Anonymous'
//                   return (
...
//                           <div className={styles.reviewUser}>{rUsername}</div>
...
//                       {(r.likesCount ?? 0) > 0 && <div className={styles.reviewLikes}>♥ {r.likesCount}</div>}
//                     </div>
//                   )
//                 })}
//               </div>
...
//     //   old: [internalRating, count]
//     //   new: { stars: 4.5, count: 12, percentage: 30 }
//     let starVal, countVal
//     if (Array.isArray(item)) {
//       starVal = item[0]
//       countVal = item[1]
//     } else if (item && typeof item === 'object') {
//       starVal = item.stars != null ? item.stars * 2 : 0  // convert back to internal 2-10
//       countVal = item.count ?? 0
//     } else {
//       return
//     }
//     const idx = Math.round(Number(starVal) / 2) - 1
//     if (idx >= 0 && idx < 5) bars[4 - idx].count += Number(countVal) || 0
//   })
...
// //   if (editingReview) {
// //     await updateReview.mutateAsync({ reviewId: editingReview.id, content: reviewText, hasSpoiler: reviewSpoiler })
// //   } else {
// //     await createReview.mutateAsync({ bookId: externalId, content: reviewText, hasSpoiler: reviewSpoiler })
// //   }
...
export default function BookDetailPage() {
  ...
  const { data: book, isLoading, isError } = useBook(externalId)
  const { data: ratingDist } = useRatingDistribution(externalId)
  const { data: reviewsData } = useBookReviews(externalId, 20)
  const logBook = useLogBook()
  const createReview = useCreateReview()
  const updateReview = useUpdateReview()
  const deleteReview = useDeleteReview()
  ...
  const [reread, setReread] = useState(false)
  const [toast, setToast] = useState(false)
  const [toastMsg, setToastMsg] = useState('Saved to your diary!')

  // Inline review form state
  const [showReviewForm, setShowReviewForm] = useState(false)
  const [editingReview, setEditingReview] = useState(null) // the review object being edited
  ...
  async function handleQuickLog(status) {
    if (!isAuthenticated) { navigate('/login'); return }
    await logBook.mutateAsync({ bookExternalId: externalId, payload: { status } })
    setToastMsg('Saved to your diary!')
    setToast(true)
  }
  ...
  async function handleFullLog() {
    ...
    // If the user also wrote a review in the log panel, save it separately
    if (logReview.trim()) {
      try {
        if (myReview) {
          await updateReview.mutateAsync({ reviewId: myReview.id, content: logReview.trim(), hasSpoiler: false })
        } else {
          await createReview.mutateAsync({ bookId: externalId, content: logReview.trim(), hasSpoiler: false })
        }
      } catch {
        // Review save failure is non-fatal — log still saved
      }
    }
  ...
  function openNewReviewForm() {
    ...
    // Close the log panel to avoid confusion
    setShowLogPanel(false)
  }

  function openEditReviewForm(r) {
    setEditingReview(r)
    setReviewText(r.content)
    setReviewSpoiler(r.hasSpoiler ?? false)
    setReviewError('')
    setShowReviewForm(true)
  }
  ...
  const reviewPending = createReview.isPending || updateReview.isPending
  ...
  return (
    <div className={styles.wrap}>
        ...
        <div className={styles.mainCol}>
          ...
          <section className={`${styles.section} fade-up`}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
              ...
              {/* Show "Write a review" button only if logged in and user hasn't reviewed yet and form isn't open */}
            ...
            {/* ── INLINE REVIEW FORM (new or edit) ── */}
            {showReviewForm && (
              <div className={`${styles.logPanel} fade-up`} style={{ marginBottom: 24 }}>
                <div className={styles.logPanelHeader}>
                  <h3 className={styles.logPanelTitle}>
                    {editingReview ? 'Edit your review' : `Review "${book.title}"`}
                  </h3>
                </div>

                <div className={styles.logField}>
                  <label className={styles.logLabel}>Your review</label>
                  <textarea
                    className={styles.logTextarea}
                    placeholder="Share your thoughts on this book…"
                    rows={5}
                ...
                {reviewError && (
                  <p className={styles.logError}>{reviewError}</p>
                )}

                <div className={styles.logActions}>
                  <button className={styles.btnGhost} onClick={cancelReviewForm} disabled={reviewPending}>
                    Cancel
                  </button>
                  <button
            ...
            {reviews.length > 0 ? (
              <div className={styles.reviewsList}>
                {reviews.map(r => {
                  const rUsername = r.username ?? r.user?.username ?? 'Anonymous'
                  const isMyReview = user?.id && r.userId === user.id
                  return (
                    <div key={r.id} className={styles.reviewCard}>
                      <div className={styles.reviewTop}>
                        <div className={styles.reviewAvatar}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 8 }}>
                        {(r.likesCount ?? 0) > 0
                          ? <div className={styles.reviewLikes}>♥ {r.likesCount}</div>
                          : <div />
                        }

                        {/* Edit / Delete buttons — only for the current user's review */}
                        {isMyReview && (
                          <div style={{ display: 'flex', gap: 8 }}>
                            <button
                              onClick={() => openEditReviewForm(r)}
      ...
      <Toast message={toastMsg} visible={toast} onHide={() => setToast(false)} />
    </div>
  )
}
